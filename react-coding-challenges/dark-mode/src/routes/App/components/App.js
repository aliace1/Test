...
function App() {
  const [darkMode, setDarkMode] = useState(false); {/* State mode */ }

  const handleClick = () => {
    setDarkMode(!darkMode);

    console.log('*************Dark Mode************', setDarkMode(!darkMode))
  }
  ...
  return (
    <div className={darkMode ? `appDark` : `app`}>
      <div className="level">
        ...
        <button className="app__dark-mode-btn icon level-right" onClick={handleClick}> {/* Button change mode boolean */}
